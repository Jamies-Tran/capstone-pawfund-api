...
public interface IShelterAssignmentPathV1API {
    ...
    @PreAuthorize("hasAnyRole({'ROLE_STAFF', 'ROLE_SHELTER_OWNER'})")
    @Operation(
            summary = "Từ chối phân công trung tâm cứu trợ",
            description = """
                    - Từ chối phân công trung tâm cứu trợ
                    - [Staff|Shop Owner - Nhân viên | Chủ trung tâm cứu trợ]
                    """)
    ValueResponse<ShelterAssignmentResponse> rejectShelterAssignment(
            @PathVariable
            Long shelterAssignmentId,

            @Valid @RequestBody
            ShelterAssignmentRejectReason rejectReason
    );
