...
public record AccountRequest(
        ...
        public AccountRequest {
                List<CommonMediaRequest> newMedias = new ArrayList<>();
                if(CollectionUtils.hasElement(medias)) {
                        List<CommonMediaRequest> thumbnails = medias.stream()
                                .filter(media -> BooleanUtils.isNotNull(media.isThumbnail())
                                        && BooleanUtils.isTrue(media.isThumbnail()))
                                .toList();

                        if(CollectionUtils.isNullOrEmpty(thumbnails) || thumbnails.size() > 1) {
                                for(int i = 0; i < medias.size(); i++) {
                                        if(i == 0) {
                                                newMedias.add(medias.get(i).withIsThumbnail(true));
                                        } else {
                                                newMedias.add(medias.get(i).withIsThumbnail(false));
                                        }
                                }
                        }
